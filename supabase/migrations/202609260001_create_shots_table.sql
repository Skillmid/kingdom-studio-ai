...
-- Migration: Create production-owned shots
-- =====================================================
...
CREATE TABLE IF NOT EXISTS public.shots (
    ...
    provenance TEXT NOT NULL DEFAULT 'user'
        CHECK (provenance IN ('user', 'scene-derived', 'ai-proposal')),
...
CREATE INDEX IF NOT EXISTS idx_shots_production
    ON public.shots(production_id);

CREATE INDEX IF NOT EXISTS idx_shots_scene
    ON public.shots(scene_id);

CREATE INDEX IF NOT EXISTS idx_shots_location
    ON public.shots(location_id);

CREATE INDEX IF NOT EXISTS idx_shots_status
    ON public.shots(status);

CREATE INDEX IF NOT EXISTS idx_shots_production_scene_number
    ON public.shots(production_id, scene_id, shot_number);

DROP TRIGGER IF EXISTS update_shots_updated_at
    ON public.shots;
...
DROP POLICY IF EXISTS "Users can view shots in their productions"
    ON public.shots;
...
DROP POLICY IF EXISTS "Users can create shots"
    ON public.shots;
...
DROP POLICY IF EXISTS "Users can update shots"
    ON public.shots;
...
DROP POLICY IF EXISTS "Users can delete shots"
    ON public.shots;
...
COMMENT ON TABLE public.shots IS
    'Production-owned camera shots planned from scenes or created by the filmmaker.';
COMMENT ON COLUMN public.shots.source_evidence IS
    'Screenplay or scene text that supports this shot proposal.';
COMMENT ON COLUMN public.shots.user_approved IS
    'Filmmaker-approved shots are preserved across later planning runs.';
