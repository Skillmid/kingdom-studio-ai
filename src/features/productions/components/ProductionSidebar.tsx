    ...
    <aside className="sticky top-0 h-screen w-72 shrink-0 border-r border-zinc-800 bg-zinc-900">
      ...
      <nav className="h-[calc(100vh-93px)] space-y-1 overflow-y-auto p-4">
        {navigation.map((item) => {
          ...
          return <Link key={item.label} href={href} className={`block rounded-xl px-4 py-3 transition ${active ? "bg-yellow-500 text-black" : "text-zinc-300 hover:bg-zinc-800 hover:text-white"}`}><div className="font-medium">{item.label}</div><div className={`mt-1 text-xs ${active ? "text-black/60" : "text-zinc-500"}`}>{item.description}</div></Link>;
        })}
      </nav>
    </aside>
  );
