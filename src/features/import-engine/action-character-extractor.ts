...
function isLikelyActionCharacterToken(
  line: string,
  token: string,
  index: number
): boolean {
  ...
  if (NON_CHARACTER_WORDS.has(token)) return false;

  // Pure uppercase lines are commonly screenplay labels, verses, or UI-like text.
  // We only accept an uppercase name when it participates in a natural action sentence.
 ...
 * This supplements dialogue cues for characters who are present in the story
 * but never speak, while remaining conservative about headings, transitions,
 * verses, notifications, and action phrases such as "DAVID TYPES".
 */
export function extractActionCharacterNames(actionLines: string[]): string[] {
  ...
  for (const line of actionLines) {
    ...
    if (!trimmed || isSceneHeading(trimmed) || isScreenplayTransition(trimmed)) {
      continue;
    }
    ...
    for (const match of trimmed.matchAll(ACTION_CHARACTER_TOKEN_PATTERN)) {
      ...
      if (!isLikelyActionCharacterToken(trimmed, token, index)) {
        continue;
      }

      const name = toTitleCaseName(token);
